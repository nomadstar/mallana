...
static bool test_state_load(struct llama_model * model, const struct common_params & params, const llama_tokens & tokens, const llama_tokens & expected_result) {
    ...
    LOG_DBG("%s: loaded state with %zu tokens\n", __func__, n_token_count_out);
...
static bool test_seq_cp_host(struct llama_model * model, const struct common_params & params, const llama_tokens & tokens, const llama_tokens & expected_result) {
    ...
    // Migrate KV cache from seq 0 to seq 1 (CPU path)
    {
        ...
        LOG_DBG("%s: seq 0 copied, %zd bytes\n", __func__, ncopy);

        llama_memory_clear(llama_get_memory(ctx.get()), true);
        LOG_DBG("%s: kv cache cleared\n", __func__);
        ...
        LOG_DBG("%s: seq 1 restored, %zd bytes\n", __func__, nset);
    }
...
// Test 4 (seq copy via the on-device path, LLAMA_STATE_SEQ_FLAGS_ON_DEVICE) is
// omitted: this fork does not implement the on-device state migration API.
...
int main(int argc, char ** argv) {
    ...
    if (params.n_parallel == 1) {
        LOG_DBG("%s: n_parallel == 1, enabling unified kv cache\n", __func__);
