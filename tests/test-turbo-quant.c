...
extern void     turbo_cpu_iwht_exported(float * x, int group_size);
extern void     turbo_cpu_de_rotate_turbo4(const float * x, float * y, int d);
...
int main(void) {
    ...
    turbo_cpu_iwht_exported(output, d);
    ...
    float temp_output[128];
    turbo_cpu_de_rotate_turbo4(output, temp_output, d);
    memcpy(output, temp_output, sizeof(output));
